...
const faqs = [
  { q: "Which AI models does PromptAI support?", a: "We support all major frontier models: ChatGPT (OpenAI), Claude (Anthropic), Gemini (Google), Midjourney, Stable Diffusion, Flux, Adobe Firefly, Cursor, Lovable, Emergent, Sora and ElevenLabs — with more added weekly." },
    ...
    <section id="faq" className="relative py-24" data-testid="faq-section">
      <div className="mx-auto max-w-3xl px-4">
        <div className="text-center">
          <p className="text-xs uppercase tracking-[0.25em] text-cyan">Questions</p>
        ...
        <Accordion type="single" collapsible className="mt-10 divide-y divide-white/5" data-testid="faq-accordion">
          {faqs.map((f, i) => (
            <AccordionItem key={i} value={`item-${i}`} className="border-b border-white/5">
              <AccordionTrigger className="text-left text-base sm:text-lg font-display tracking-tight hover:no-underline" data-testid={`faq-q-${i}`}>
                {f.q}
              </AccordionTrigger>
              <AccordionContent className="text-[#94A3B8] text-[15px] leading-relaxed" data-testid={`faq-a-${i}`}>
                {f.a}
              </AccordionContent>
            </AccordionItem>
          ))}
